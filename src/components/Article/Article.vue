    ...
    <ul>
      <li class="vux-1px-b" v-bind:class="{ large: item.view_mode=='L', small: item.view_mode=='T' }" v-for="(item,index) in articlelist" @click="todetail(item.item)" :key="index">
        ...
        <div v-else class="thumb">
          <img :src="item.thumb" alt="picture" onerror="this.src='http://m.51xy8.com/static/img/default.png'">
        </div>
        <p class="title" v-html="item.item_name"></p>
        <div class="author">
          <img class="author_pic" :src="item.author_pic" alt="">
          <span v-html="item.author_name"></span>
          <span class="data">{{item.postd | formatDate}}</span>
        </div>
      </li>
    </ul>
  </div>
