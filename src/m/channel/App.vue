<template>
  <div id="channel">
      <div class="content" v-if="showContent">
        ...
        <div class="buy_member" v-if="!subscription&&channelsinfo.member_prices&&channelsinfo.member_prices.length!=0">
          ...
          <p class="buy-now" @click="toActiveMember">
            &nbsp;&nbsp;购买会员
				...
				<share-money-btn type="1" :id="id" :number="channelsinfo.gift_tip?channelsinfo.gift_tip:0" :moneyVal="channelsinfo.share_make_money?channelsinfo.share_make_money:0"></share-money-btn>
        ...
        <!-- 支付行为 -->
        <buy-action v-if="showContent&&!subscription" ref="buy_action" :shareFrom="shareFrom" :isCard="isCard" :sourceFrom="sourceFrom" :name="channelsinfo.name" :buyParams="buy_params" :coupons="channelsinfo.coupons" :invalidCoupons="channelsinfo.invalid_coupons" :composite="channelsinfo.composite" :backurl="backurl" v-on:errorFun="errorFun"> </buy-action>
      </div>
    <!-- 底部弹框 -->
